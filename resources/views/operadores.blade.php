...
<div class="card" id="dvGrid">
  <div class="card-header" style="background-color: #fff; position: absolute; top: -90px; right: 0px; border: 0px;">
    <button type="button" id="btNewRegister" class="btn btn-success">Novo Operador</button>
  </div>
  <!-- /.card-header -->
  <div class="card-body">
...
$(function() {
    ...
    table.on('click', '.btedit', function () {
        ...
        console.log(selid);
        $.getJSON("/api/operador/"+selid, function(data){
            $('#dvGrid').hide();
            $('#theForm')[0].reset();
    ...
    table.on('click', '.btdelete', function () {
        selid = table.row($(this).closest('tr')).data().id;
        bootbox.confirm({
            message: "Deseja mesmo apagar esse operador?",
            buttons: {
                cancel: {
                    label: 'Não',
            ...
            callback: function (result) {
                if (result) {
                    $.get("/api/operador/"+selid+"/apagar",
                    {}
                    ,function(data){
                        if (data.status == 'success') {
                            bootbox.alert('Operador removido com sucesso!', function(){
                                $.acShowGrid();
                            });
                        }
                    });
    ...
    $('#theForm').ajaxForm({
        ...
        success: function() {
            bootbox.alert('Operador salvo com sucesso', function(){
                $.acShowGrid();
            });
        }
    });
